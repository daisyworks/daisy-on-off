...
{
  private final int gpioMask;
  private final int pioMask;

  public ButtonState(final int gpioMask, final int pioMask)
  {
    this.gpioMask = gpioMask;
    this.pioMask = pioMask;
  }

  public boolean isPinHigh(final int pin)
  {
    if (pin < 9)
    {
      return ((1 << pin ) & gpioMask)  > 0;
    }
    return ((1 << (pin - 8)) & pioMask) > 0;
  }
}